...
def find_quadrilateral(edges, debug_plotting=False):
    ...
    peaks = hough_line_peaks(
        h, theta, d, num_peaks=20, threshold=0.5 * np.min(edges.shape)
    )
    if debug_plotting:
...
def _find_quadrilateral(peaks, origin, score_image):
    ...
    for l1, l2 in itertools.combinations(lines, 2):
        try:
            ...
            if angle_deg < 60:
                continue
            if x < -0.1 * score_image.shape[1] or x >= 1.1 * score_image.shape[1]:
                continue
            if y < -0.1 * score_image.shape[0] or y >= 1.1 * score_image.shape[0]:
                continue
    ...
    def gen_scored_points():
        ...
        for points in itertools.combinations(intersection_points, 4):
            points = np.array(
                list(map(np.array, sorted(points, key=lambda x: -sum(x))))
            )
