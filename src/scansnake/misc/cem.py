...
class CEMMinimizer(object):
    ...
    def minimize(self, f):
        ...
        for _ in range(self.niter):
            ...
            elites = sorted(
                ((td, s) for td, s in zip(scores, S) if td <= baseline),
                key=lambda x: x[0],
            )[: int(self.relite * self.nsamples)]
...
def test():
    ...
    x = CEMMinimizer(
        distribution=make_normal(mu, Sigma),
        infer_distribtion=infer_normal,
        nsamples=N,
        verbose=False,
    ).minimize(tour_distance)

    # draw_tour(x, problem)
    print("min distance ", tour_distance(x))
    x = CEMMinimizer(
        distribution=make_uniform(np.zeros(n_problem), np.ones(n_problem)),
