...
from cem import CEMMinimizer, infer_normal, infer_uniform, make_normal, make_uniform
...
def find_perp_distance(p0, p1, parr, perp, sign):
    ...
    # n01 = n01 / d01
    d01perp = np.array([[0.0, -1.0], [1.0, 0.0]]) @ d01
...
def test():
    ...
    im = np.array(
        Image.open("/Users/MichaelMason/Desktop/original-small.jpg").convert("L")
    )
    ...
    minimizer = CEMMinimizer(
        distribution=make_uniform(np.zeros(8), np.ones(8)),
        niter=50,
        infer_distribtion=infer_uniform,
        verbose=True,
        nsamples=500,
        relite=0.1,
    )
...
def test2():
    ...
    plt.imshow(edges)
    for acc, angle, dist in zip(
        *hough_line_peaks(h, theta, d, min_distance=1, min_angle=1, num_peaks=16)
    ):
...
def test3():
    ...
    s = np.array(
        sorted(
            np.array(
                list(np.random.normal(size=nobs))
                + list(np.random.normal(size=nobs) + 10)
            )
        )
    )
